...
import { FakeBackend_Users } from "../fake-backend/users";
...
export class AuthService {
  ...
  private getUserIndex(usersArr: any[], usersStore: string, usernameInput: string){
    let getIndex: number = -1;
    for(var i = 0; i < usersArr.length; i += 1) {
  ...
  private isUserAuthenticated(users: any[], usersKeyName: string, userInput: string, index: any){
    return users[index][usersKeyName] === userInput;
  }

  /// getting a key value from authenticated users
  private getName(users: any[], index: any, key: string) {
